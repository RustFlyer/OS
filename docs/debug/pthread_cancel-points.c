...
{
	TESTR(1, "sub-thread enters run_execute");
	pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, 0);
	while (sem_wait(&sem_seq));
	TESTR(1, "sub-thread passes sem_wait");
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, 0);
	seqno = 1;
	TESTR(1, "sub-thread trys to block wait(want to be interrupted)");
...
int main(void)
{
	...
	for (; cur_sc->prepare; cur_sc++) {
		...
		seqno = 0;
		TESTR(1, "main-thread plans to create thread");
		TESTR(pthread_create(&td, 0, run_execute, 0), "creating thread to be canceled");
		TESTR(1, "main-thread plans to cancel thread");
		TESTR(pthread_cancel(td), "canceling");
		TESTR(1, "main-thread plans to post sem(tick sem_wait in sub-thread)");
		TESTR(sem_post(&sem_seq), "unblocking canceled thread");
		TESTR(1, "main-thread plans to pthread_join");
