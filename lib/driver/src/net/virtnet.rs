use alloc::{boxed::Box, sync::Arc, vec::Vec};
use core::{any::Any, fmt::Debug};

use smoltcp::phy::{DeviceCapabilities, Medium};
use virtio_drivers::{
    device::net::VirtIONetRaw,
    transport::{Transport, mmio::MmioTransport},
};

use super::{
    DevError, DevResult, EthernetAddress, NetDevice,
    netpool::{NET_BUF_LEN, NET_BUF_POOL, NetBuf, NetBufPool, NetBufPtr, NetBufPtrOps},
};
use crate::hal::VirtHalImpl;

#[allow(clippy::vec_box)]
pub struct VirtIoNetDev<T: Transport, const QS: usize> {
    rx_buffers: [Option<Box<NetBuf>>; QS],
    tx_buffers: [Option<Box<NetBuf>>; QS],
    free_tx_bufs: Vec<Box<NetBuf>>,
    device: VirtIONetRaw<VirtHalImpl, T, QS>,
    capabilities: DeviceCapabilities,
}

impl<T, const QS: usize> VirtIoNetDev<T, QS>
where
    T: Transport,
{
    /// Creates a new VirtIO network device with the given [`Transport`].
    pub fn build(transport: T) -> DevResult<Self> {
        const NONE_BUF: Option<Box<NetBuf>> = None;

        let rx_buffers = [NONE_BUF; QS];
        let tx_buffers = [NONE_BUF; QS];
        let free_tx_bufs = Vec::with_capacity(QS);

        let mut capabilities = DeviceCapabilities::default();
        capabilities.medium = Medium::Ethernet;
        capabilities.max_transmission_unit = NET_BUF_LEN;
        capabilities.max_burst_size = None;

        let device = VirtIONetRaw::<VirtHalImpl, T, QS>::new(transport)
            .map_err(DevError::from_virtio_error)?;

        let mut dev = Self {
            rx_buffers,
            device,
            tx_buffers,
            free_tx_bufs,
            capabilities,
        };

        // Allocate receive buffers and submit them to the device.
        for (i, rx_buf_place) in dev.rx_buffers.iter_mut().enumerate() {
            let mut rx_buf = NET_BUF_POOL.alloc().ok_or(DevError::NoMemory)?;
            let token = unsafe {
                dev.device
                    .receive_begin(rx_buf.buffer_mut())
                    .map_err(DevError::from_virtio_error)?
            };
            assert_eq!(token, i as u16);
            *rx_buf_place = Some(Box::new(rx_buf));
        }

        // Allocate transmit buffers, fill the header, and add them to the free list.
        for _ in 0..QS {
            let mut tx_buf = NET_BUF_POOL.alloc().ok_or(DevError::NoMemory)?;
            let header_len = dev
                .device
                .fill_buffer_header(tx_buf.buffer_mut())
                .or(Err(DevError::InvalidParam))?;
            tx_buf.set_header_len(header_len);
            dev.free_tx_bufs.push(Box::new(tx_buf));
        }

        Ok(dev)
    }
}

unsafe impl<T: Transport, const QS: usize> Send for VirtIoNetDev<T, QS> {}
unsafe impl<T: Transport, const QS: usize> Sync for VirtIoNetDev<T, QS> {}

impl<T, const QS: usize> NetDevice for VirtIoNetDev<T, QS>
where
    T: Transport,
{
    fn capabilities(&self) -> DeviceCapabilities {
        self.capabilities.clone()
    }

    fn mac_address(&self) -> EthernetAddress {
        EthernetAddress(self.device.mac_address())
    }

    fn can_transmit(&self) -> bool {
        !self.free_tx_bufs.is_empty() && self.device.can_send()
    }

    fn can_receive(&self) -> bool {
        self.device.poll_receive().is_some()
    }

    fn rx_queue_size(&self) -> usize {
        QS
    }

    fn tx_queue_size(&self) -> usize {
        QS
    }

    fn recycle_rx_buffer(&mut self, rx_buf: Box<dyn NetBufPtrOps>) -> DevResult {
        let rx_buf =
            unsafe { core::mem::transmute::<Box<dyn NetBufPtrOps>, Box<dyn Any + Send>>(rx_buf) };
        let mut rx_buf = unsafe { NetBuf::from_buf_ptr(*rx_buf.downcast::<NetBufPtr>().unwrap()) };
        // Safe because we take the ownership of `rx_buf` back to `rx_buffers`,
        // it lives as long as the queue.
        let new_token = unsafe {
            self.device
                .receive_begin(rx_buf.buffer_mut())
                .map_err(DevError::from_virtio_error)?
        };
        // `rx_buffers[new_token]` is expected to be `None` since it was taken
        // away at `Self::receive()` and has not been added back.
        if self.rx_buffers[new_token as usize].is_some() {
            return Err(DevError::BadState);
        }
        self.rx_buffers[new_token as usize] = Some(rx_buf);
        Ok(())
    }

    fn recycle_tx_buffers(&mut self) -> DevResult {
        while let Some(token) = self.device.poll_transmit() {
            let tx_buf = self.tx_buffers[token as usize]
                .take()
                .ok_or(DevError::BadState)?;
            unsafe {
                self.device
                    .transmit_complete(token, tx_buf.header_and_packet())
                    .map_err(DevError::from_virtio_error)?;
            }
            // Recycle the buffer.
            self.free_tx_bufs.push(tx_buf);
        }
        Ok(())
    }

    fn transmit(&mut self, tx_buf: Box<dyn NetBufPtrOps>) -> DevResult {
        let tx_buf =
            unsafe { core::mem::transmute::<Box<dyn NetBufPtrOps>, Box<dyn Any + Send>>(tx_buf) };
        // 0. prepare tx buffer.
        let tx_buf = unsafe { *NetBuf::from_buf_ptr(*tx_buf.downcast::<NetBufPtr>().unwrap()) };
        // 1. transmit packet.
        let token = unsafe {
            self.device
                .transmit_begin(tx_buf.header_and_packet())
                .map_err(DevError::from_virtio_error)?
        };
        self.tx_buffers[token as usize] = Some(Box::new(tx_buf));
        Ok(())
    }

    fn receive(&mut self) -> DevResult<Box<dyn NetBufPtrOps>> {
        if let Some(token) = self.device.poll_receive() {
            log::warn!("[VirtioNetDev::receive] token {}", token);
            let mut rx_buf = self.rx_buffers[token as usize]
                .take()
                .ok_or(DevError::BadState)?;

            let (hdr_len, pkt_len) = unsafe {
                self.device
                    .receive_complete(token, rx_buf.buffer_mut())
                    .map_err(DevError::from_virtio_error)?
            };
            rx_buf.set_header_len(hdr_len);
            rx_buf.set_packet_len(pkt_len);

            Ok(Box::new(rx_buf).into_buf_ptr())
        } else {
            Err(DevError::Again)
        }
    }

    fn take_tx_buffer(&mut self, size: usize) -> DevResult<Box<dyn NetBufPtrOps>> {
        let mut buffer = self.free_tx_bufs.pop().ok_or(DevError::NoMemory)?;
        if size + buffer.header_len() > buffer.capacity() {
            self.free_tx_bufs.push(buffer);
            return Err(DevError::InvalidParam);
        }
        buffer.set_packet_len(size);
        Ok(Box::new(buffer).into_buf_ptr())
    }
}

pub type VirtIoNetDevImpl = VirtIoNetDev<MmioTransport<'static>, 32>;

pub fn create_virt_net_dev(transport: MmioTransport<'static>) -> DevResult<Box<VirtIoNetDevImpl>> {
    Ok(Box::new(VirtIoNetDevImpl::build(transport)?))
}

impl Debug for VirtIoNetDevImpl {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("VirtIoNetDevImpl")
            .field("rx_buffers", &self.rx_buffers)
            .field("tx_buffers", &self.tx_buffers)
            .field("free_tx_bufs", &self.free_tx_bufs)
            .finish()
    }
}
