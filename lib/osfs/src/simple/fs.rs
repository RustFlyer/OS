...
impl SimpleFsType {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            meta: FileSystemTypeMeta::new("simplefs"),
        })
    }
}
...
impl FileSystemType for SimpleFsType {
    ...
    fn base_mount(
        ...
        let sb = SimpleSuperBlock::new(dev, self.clone());
...
impl SimpleSuperBlock {
    pub fn new(
        device: Option<Arc<dyn BlockDevice>>,
        fs_type: Arc<dyn FileSystemType>,
    ) -> Arc<Self> {
        Arc::new(Self {
            meta: SuperBlockMeta::new(device, fs_type, 0x999),
        })
    }
}

impl SuperBlock for SimpleSuperBlock {
