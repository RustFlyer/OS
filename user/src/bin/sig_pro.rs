...
fn user_sig_test_failsignum() {
    ...
    if sigaction(500.into(), &new, &mut old) >= 0 {
...
fn user_sig_test_kill() {
    ...
    if kill(getpid(), Sig::SIGUSR1.raw()) < 0 {
        println!("Kill failed!");
        exit(1);
    }
}
...
fn user_sig_test_restore() {
    ...
    if sigaction(Sig::SIGUSR1, &old, &mut old2) < 0 {
        panic!("Sigaction failed!");
    }
...
//     if kill(getpid(), SigSet::SIGSTOP.bits() as usize) < 0 {
//         println!("kill faild\n");
//         exit(-1);
//     }
// }
...
fn kernel_sig_test_stop_cont() {
    let pid = fork();
    if pid == 0 {
        kill(getpid(), Sig::SIGSTOP.raw());
        sleep(500);
        exit(-1);
    } else {
        sleep(1000);
        kill(pid, Sig::SIGCONT.raw());
        let mut exit_code = 0;
        wait(&mut exit_code);
    }
}
...
fn kernel_sig_test_failignorekill() {
    ...
    let mut unused = SigAction::default();
    if sigaction(Sig::SIGKILL, &new, &mut unused) >= 0 {
        panic!("Should not set sigaction to kill!");
    }

    if sigaction(Sig::SIGKILL, &mut unused, &mut old) >= 0 {
        panic!("Should not set sigaction to kill!");
    }
}
